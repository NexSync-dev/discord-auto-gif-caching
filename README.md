...
A plugin for [Equicord](https://github.com/Equicord/Equicord) / [Vencord](https://github.com/Vendicated/Vencord) that caches your favorited GIFs locally so they load from memory instead of re-fetching from Discord's CDN every time.

---

## ✨ Features

- GIFs load instantly from memory on subsequent opens
- Cache persists across Discord restarts via IndexedDB
- All favorites are preloaded 5 seconds after Discord launches
- Newly favorited GIFs are cached immediately when you heart them
- Listens to `USER_SETTINGS_PROTO_UPDATE` so favorites added on mobile/another device get cached too
- Periodically re-reads your favorites list to catch anything new
- Built-in cache inspector in plugin settings: see every cached GIF, its size, cache date, and delete individual entries or wipe everything
...
| Max cache entries | `200` | Oldest entries are pruned automatically when the limit is hit |

The cache inspector below the settings shows total GIF count and storage size. Per-entry controls let you delete individual GIFs; **↻ Refresh** reloads the list and **🗑 Clear All** wipes everything.

---

## 🗂 Where is the database?

IndexedDB inside Discord's Electron browser context. To inspect it manually:
...
Each entry stores the GIF blob, URL, and a timestamp.

---

## ⚠️ Known limitations

- Tenor proxy URLs (`images-ext-1.discordapp.net`) are skipped — Discord's CDN proxy for Tenor blocks cross-origin fetches. Direct Tenor URLs (`media.tenor.com`) and Discord attachment GIFs work fine.
- Discord CDN attachment URLs contain expiry tokens (`?ex=...`). If a URL expired before caching, it fails silently and retries on the next refresh cycle.
- The plugin does not patch Discord's webpack or intercept the renderer, so it won't break Discord's UI.
